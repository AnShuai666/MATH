...
//LU求逆(将每列b求出的各列x进行组装)
template<typename Tp>
bool luSolveInverse(const Tp* A,int n,float* inv_A)
{
    int DN=n;
    //创建矩阵A的副本，注意不能直接用A计算，因为LUP分解算法已将其改变
    float *A_mirror   = new float[DN * DN]();
